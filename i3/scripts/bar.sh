...
volume_status() {
    # Get volume and mute status from wpctl for the default audio sink
    ...
    if echo "$status" | grep -q "MUTED"; then
        ...
        vol=$(echo "$status" | awk '{print $2 * 100}' | cut -d'.' -f1)
        echo "VOL: ${vol}%"
    fi
}
...
media_status() {
    ...
    if command -v playerctl &> /dev/null; then
        ...
        artist=$(playerctl metadata artist 2>/dev/null | head -c 100)
        title=$(playerctl metadata title 2>/dev/null | head -c 90)
        
        if [ -n "$artist" ] && [ -n "$title" ]; then
            # Truncate if too long
            if [ ${#artist} -eq 90 ]; then
                artist="${artist}..."
            fi
            if [ ${#title} -eq 100 ]; then
                title="${title}..."
            fi
            current_media="$symbol $artist - $title"
        elif [ -n "$title" ]; then
